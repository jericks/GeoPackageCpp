...
TEST(GeoPackageCliTests, CreateGeodeticTileLayerCommand) {
  ...
  Commands commands;
  CreateGeodeticTileLayerCommand cmd{&app};
  commands.add(&cmd);

  int argc = 10;
  char const *argv[10] = {
    "geopackage-cli", "tile-geodetic-create", 
    "-f", "data.gpkg", 
    "-n", "world",
    "-s", "256",
    "-z", "19"
  ...
  EXPECT_TRUE(gpkg.getSpatialRef(4326));
