...
namespace geopackage {
    ...
    Content::Content(std::string tableName, std::string dataType, std::string identifier, std::string description, std::string lastChange, Bounds bounds, int srsId) :
    ...
    std::string Content::getDataType() const {
        return dataType;
    }
    ...
    std::ostream& operator << (std::ostream& os, const Content& e) {
        os << "CONTENT (tableName = " << e.getTableName() << ", dataType = " << e.getDataType()
